...
class EventLoop : public NonCopyable {
 public:
  ...
  /**
   * Unregisters a nanoapp from receiving broadcast events of a given type and
   * target group mask.
   *
   * @param instanceId The instance ID of the nanoapp to unregister.
   * @param eventType The event type to unregister.
   * @param targetGroupMask The target group mask to unregister.
   *
   * @see Nanoapp::unregisterForBroadcastEvent
   *
   */
  void unregisterEventForNanoapp(uint16_t instanceId, uint16_t eventType,
  ...
  /**
   * Returns the next instance ID to be used by this event loop.
   *
   * @return The next instance ID to be used by this event loop.
   */
  uint16_t getNextNanoappInstanceId();

  /**
   * Returns the event that is currently being freed by this event loop.
   *
   * @return A pointer to the currently freeing event loop, or null if no events
   * are being freed.
   */
  const Event *getCurrentFreeingEvent() {
    return mCurrentFreeingEventStack.empty() ? nullptr
                                             : mCurrentFreeingEventStack.back();
  }
 ...
 private:
  ...
  uint32_t mNextInstanceId{kSystemInstanceId + 1};

  //! A stack-based list of events that are being freed. This list may
  //! contain multiple elements if multiple events are being freed
  //! in this event loop (for example, if a free callback frees another event).
  static constexpr size_t kMaxFreeingEventStack =
      CHRE_MAX_FREEING_EVENT_STACK_SIZE;
  FixedSizeVector<Event *, kMaxFreeingEventStack> mCurrentFreeingEventStack;
